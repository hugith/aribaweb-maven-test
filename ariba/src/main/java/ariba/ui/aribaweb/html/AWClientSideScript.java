...
public final class AWClientSideScript extends AWComponent {
	...
	private static final String[] SupportedBindingNames = { BindingNames.scriptFile, BindingNames.scriptString, BindingNames.language, BindingNames.filename, BindingNames.elementId, BindingNames.invokeAction, RJSMayNotBeDefined, isSingleton, executeOnIncrementalUpdate, globalScope, executeOn, handle, BindingNames.forceDirectInclude, BindingNames.synchronous };
	...
	public boolean useDirectInclude() {
		return !AWPage.AllowIncrementalScriptLoading() || booleanValueForBinding(BindingNames.forceDirectInclude) || !nullOrFalse(env().peek(AWBindingNames.scriptForceDirectInclude));
	}
	...
	public void renderResponse(AWRequestContext requestContext, AWComponent component) {
		// if this clientsidescript is for a .js resource, then we need to have
		// a full
		// page refresh so the methods, global vars, etc. get registered in the
		// main
		// window rather than the refresh iframe. We could copy all these from
		// the
		// refresh iframe over to the main window, but there are additional
		// issues there
		// that we have to solve. Since we can't incrementally render a full
		// page
		// refresh -- ie, figure out during diffing that we need a fpr and then
		// bust out
		// of the diff mode and send a fpr (see AWPage, AWResponseBuffer, etc.)
		// -- we do
		// the following test.
		if (_filename != null) {
			// Our basic strategy is to force a FPR the first time we see a
			// script
			// rendered on a page. The corner case that causes problems with a
			// single
			// script tracking list is the following --
			// * p1 displayed and script1 rendered, then script1 placed in p1
			// script list
			// * subsequent redisplay of p1 causes fpr (p1'), but p1' does not
			// contain
			// script1. (thus at this point, p1' does not contain the script1,
			// but the
			// script list on the server still indicates that script1 has been
			// registered on the page)
			// * incremental update of p1' (p1'') once again contains script1,
			// but since
			// script1 is contained in p1 script list, we don't trigger the FPR
			// required
			// for script1 to once again be registered on the main window.

			// Solve this by using two script lists --
			// page script list -- what is currently registered on the page on
			// the client
			// current script list -- script list from current rendering of page
			// if a FPR occurs, then copy current script list to page script
			// list.
			// use page script list when determining whether or not to force a
			// FPR

			// if this is the first time we have rendered the script, then force
			// a FPR
			if (!page().hasScript(_filename)) {
				if (!requestContext().fullPageRefreshRequired() && !useDirectInclude()) {
					Log.domsync.debug("Adding new script to page: %s (page:%s, list:%s)", _filename, page(), page()._pageScriptList());
				}
				if (!AWPage.AllowIncrementalScriptLoading())
					requestContext().forceFullPageRefresh();
			}
			// always track the list of current scripts -- in case of a FPR, we
			// replace
	...
	public String scriptFileUrl() {
		String scriptFileUrl = (((AWConcreteApplication) application()).allowsJavascriptUrls()) ? urlForResourceNamed(_filename, false, true) : AWXDebugResourceActions.urlForResourceNamed(requestContext(), _filename);
